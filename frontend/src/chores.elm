...
import Json.Decode exposing (Decoder, field, dict, map2, int)
...
type alias Schedule = {chores : Dict String Chore, timer: Time.Posix}

type alias Chore = {start: Int, threshold : Int}
...
update msg model =
    case msg of
        ...
        Reset chore_name -> (model, updateChore chore_name)
        ChoreConf result ->
            case result of
                Ok chores ->
        ...
        ChoreReset _ -> (model, getChoreConf)
...
view model =
    case model of
        ...
        Success schedule -> div [] (displayChores schedule)
